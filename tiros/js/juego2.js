...
/*		{left:128, top:164,width:25,height:25,relativo:true},
		{left:159, top:120,width:25,height:25,relativo:true},
		{left:190, top:75,width:24,height:24,relativo:true},
		{left:214, top:35,width:19,height:19,relativo:true},
		{left:242, top:0,width:15,height:15,relativo:true},
*/
...
game.addKeyListener(
   {
      ...
      listener: function () {
         ...
         now = +new Date();
         if (now - lastKeyListenerTime > 3000) { // throttle
            lastKeyListenerTime = now;
            if(!MOVIMIENTO_JUGADOR) {
            	...
            	if(ladoIzquierdo<=centroPelotaX && ladoDerecho>=centroPelotaX && ladoSuperior<=centroPelotaY && ladoInferior>=centroPelotaY) {
            		...
            		ladoDerecho = ladoIzquierdo + ZONA_DISPARO_WIDTH/3;
            		if(ladoIzquierdo<=centroPelotaX && ladoDerecho>=centroPelotaX) {
            			DIRECCION_TIRO=1;
            		}

            		ladoIzquierdo = jugador.left + ZONA_DISPARO_LEFT+ ZONA_DISPARO_WIDTH/3;
            		ladoDerecho = ladoIzquierdo + ZONA_DISPARO_WIDTH/3;
            		if(ladoIzquierdo<=centroPelotaX && ladoDerecho>=centroPelotaX) {
            			DIRECCION_TIRO=0;
            		}

            		ladoIzquierdo = jugador.left + ZONA_DISPARO_LEFT+ ZONA_DISPARO_WIDTH/3*2;
            		ladoDerecho = ladoIzquierdo + ZONA_DISPARO_WIDTH/3;
            		if(ladoIzquierdo<=centroPelotaX && ladoDerecho>=centroPelotaX) {
            			DIRECCION_TIRO=2;
            		}
...
function VerificarGol() {
	...
	if(ladoIzquierdo<=centroX && centroX<=ladoDerecho && ladoSuperior<=centroY && centroY<=ladoInferior) {
		if(centroY<=(ARCO_TOP+ARCO_HEIGHT/2)) {

			//game.playSound('gol', false);
			...
			//GOL = false;
			if(CANTIDAD_PELOTAS_JUEGO==1 || CANTIDAD_PELOTAS_JUEGO==2) {
				var velocidadx = 5
				var velocidady = 1;
				var factorrebote = 0.8;
				var gravedad = 0.02;
				var posicionactual = pelota.top;

				if(CANTIDAD_PELOTAS_JUEGO==2) {
					CONTADOR_REBOTES=0;
					/*var fn = function(){
							velocidady+=gravedad;
							pelota.top+=velocidady;
							pelota.left-=velocidadx;

							if(pelota.top>(posicionactual + 280)) {
							 	velocidady*=-factorrebote;
							}

							CONTADOR_REBOTES++;
							if(CONTADOR_REBOTES<8) {
								setTimeout(fn, 10);
							}
